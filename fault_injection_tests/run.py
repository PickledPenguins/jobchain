...
from jobchain.scheduler import PBS, Scheduler, _capture, write_script
...
class TestAtomicWriteFailures(TempCase):
    def test_write_text_removes_temporary_file_when_rename_fails(self):
        ...
        with patch(
            "jobchain.store.os.replace", side_effect=OSError("read-only")
    ...
    def test_write_text_preserves_old_value_when_replacement_fails(self):
        ...
        with open(target, "w", encoding="utf-8") as handle:
            handle.write("old")
        with patch("jobchain.store.os.replace", side_effect=OSError("full")), self.assertRaises(
            OSError
        ):
            _write_text(target, "new")
    ...
    def test_store_config_write_failure_does_not_leave_partial_json(self):
        ...
        with patch(
            "jobchain.store.os.replace", side_effect=OSError("I/O failure")
        ), self.assertRaises(OSError):
    ...
    def test_script_write_failure_cleans_temporary_script(self):
        ...
        with patch(
            "jobchain.scheduler.os.replace", side_effect=OSError("disk full")
        ), self.assertRaises(OSError):
...
class TestHelperFaults(TempCase):
    def test_helper_execution_failure_becomes_node_helper_error(self):
        ...
        with patch(
            "jobchain.store.subprocess.run", side_effect=OSError("exec failed")
        ), self.assertRaises(NodeHelperError):
...
class TestSchedulerFaults(TempCase):
    def test_submit_timeout_is_not_silently_treated_as_success(self):
        ...
        with patch(
            "jobchain.scheduler.subprocess.run", side_effect=subprocess.TimeoutExpired("qsub", 60)
        ), self.assertRaises(SchedulerError):
            scheduler.submit("job.sh", {})

    def test_scheduler_query_timeout_degrades_to_unknown(self):
        with patch("jobchain.scheduler.shutil.which", return_value="/bin/qstat"), patch(
            "jobchain.scheduler.subprocess.run", side_effect=subprocess.TimeoutExpired("qstat", 60)
        ):
