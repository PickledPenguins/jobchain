...
def row(name="r", status=DONE, valid=True, run=None, runs=None, work_dir=""):
    ...
    return RowState(
        ...
        index=0,
        params={"a": 1},
        generation=runs[-1].generation if runs else 0,
        runs=runs,
        valid=valid,
        invalid_reasons=[],
        failure_id="",
...
class TestReportCategoriesAndMetrics(unittest.TestCase):
    ...
    def test_metrics_invalid_cancelled_and_stage_failure(self):
        ...
        rows = [
            row("done", DONE),
            row("bad", valid=False),
            row("fail", FAILED, run=RunState(generation=1, stages=[failed])),
            row("cancel", CANCELLED, run=RunState(generation=1, stages=[cancelled])),
        ]
    ...
    def test_metrics_timestamp_min_max_updates(self):
        ...
        m = compute_metrics([r])
        self.assertEqual(m.first_event.hour, 10)
        self.assertEqual(m.last_event.minute, 5)

    def test_metrics_properties_no_finished_and_no_remaining(self):
        m = Metrics(total=1, invalid=1)
        self.assertIsNone(m.failure_rate)
        self.assertIsNone(m.eta_seconds)
        m = Metrics(total=0, completed=0, failed=0, first_event=None, last_event=None)
        self.assertIsNone(m.wall_elapsed)
    ...
    def test_metrics_to_dict_empty_stage_values(self):
        ...
        d = m.to_dict()
        self.assertIsNone(d["per_stage"]["s"]["mean_s"])
        self.assertEqual(d["per_stage"]["s"]["failures"], 2)
...
class TestStatusRendering(unittest.TestCase):
    ...
    def test_metrics_every_optional_section(self):
        ...
        m.first_event = __import__("datetime").datetime(2026, 1, 1, 10)
        m.last_event = __import__("datetime").datetime(2026, 1, 1, 11)
        m.per_stage = {"s": [30.0, 60.0], "empty": []}
        m.stage_failures = {"s": 1}
...
class TestShowRendering(unittest.TestCase):
    def test_show_invalid_failure_handoff_paths_and_history(self):
        failed = stage(
            ...
            FAILED,
            jobid="9",
            timeline=["2026-01-01 10:00:00 host=node", "2026-01-01 10:05:00 host=node status=DONE"],
            error="first\nsecond",
        )
        old = RunState(generation=1, stages=[stage("old", DONE)])
        current = RunState(generation=2, stages=[failed], handoff={"mesh": "/x"})
        r = row("a", FAILED, valid=False, runs=[old, current], work_dir="/does/not/exist")
        r.generation = 2
    ...
    def test_show_selective_sections(self):
        ...
        store = SimpleNamespace(
            home="/h", row_dir=lambda n: "/state", read_manifest=lambda n: [("a", "b", "script")]
        )
    ...
    def test_show_stage_table_with_defaults(self):
        stages = [
            stage("a", DONE, jobid="", resources={}),
            stage("b", PENDING, jobid=None, resources={"mem": "1G"}),
        ]
...
class TestExportAndTimeline(unittest.TestCase):
    def test_export_row_with_no_stage_and_invalid_reasons(self):
        ...
        r = row("a", valid=False, runs=[], work_dir="/w")
        r.invalid_reasons = ["bad"]
        lines = export_rows(schema, [r])
        self.assertIn("|bad", lines[1])
    ...
    def test_export_row_with_stage_error(self):
        st = stage(
            ...
            FAILED,
            timeline=[
                "2026-01-01 10:00:00 host=n status=RUNNING",
                "2026-01-01 10:01:00 host=n status=FAILED",
            ],
            error="bad\nmore",
        )
        schema = SimpleNamespace(field_names=["a"], delimiter="|", quoting=False)
    ...
    def test_failed_stage_none_and_match(self):
        self.assertIsNone(_failed_stage(None))
        self.assertIsNone(_failed_stage(SimpleNamespace(stages=[])))
    ...
    def test_directory_summary_walk_error(self):
        with tempfile.TemporaryDirectory() as d, patch(
            "jobchain.report.os.walk", side_effect=OSError("denied")
        ):
    ...
    def test_directory_summary_depth_limit(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            os.makedirs(nested)
            with open(os.path.join(d, "root"), "w") as h:
                h.write("x")
            with open(os.path.join(nested, "deep"), "w") as h:
                h.write("xx")
            count, size = _directory_summary(d, depth=1)
