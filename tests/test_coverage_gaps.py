...
class TestSchemaValidatorEdges(unittest.TestCase):
    def test_path_resolution_and_validator_basics(self):
        ...
        self.assertEqual(
            S.Validator.__subclasses__()[0] if False else S.Int("x").normalize(" x "), "x"
        )
    ...
    def test_row_file_and_schema_edges(self):
        ...
        self.assertEqual(
            S.RowCount(min=2).check(
                [
                    (1, {}),
                ]
            ),
            [(0, "file has 1 rows, fewer than the required 2")],
        )
        ...
        with self.assertRaises(ValueError):
            S.Unique([])
        with self.assertRaises(ValueError):
            S.RowCount(min=2, max=1) if False else S.Unique([])

    def test_schema_structure_reference_and_uniqueness(self):
        with self.assertRaises(SchemaError):
            S.Schema(name="x", fields=[])
        with self.assertRaises(SchemaError):
            S.Schema(name="x", fields=[S.Field("a"), S.Field("a")])
        with self.assertRaises(SchemaError):
            S.Schema(name="x", fields=[S.Field("a")], id_field="b")
        ...
        with self.assertRaises(SchemaError):
            S.Schema(name="x", fields=[S.Field("a")], quoting=True, delimiter=None)
        with self.assertRaises(SchemaError):
            S.Schema(
                name="x",
                fields=[S.Field("a")],
                row_validators=[S.RequiredWhen("missing", "x", "a")],
            )
        with self.assertRaises(SchemaError):
            S.Schema(name="x", fields=[S.Field("a")], file_validators=[S.Unique(["missing"])])
...
class TestConfigEdges(unittest.TestCase):
    def test_config_load_and_template_errors(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(c.provenance["width"], "cli")
            ...
            self.assertIn(
                "# Effective",
                __import__("jobchain.config", fromlist=["render_final_config"]).render_final_config(
                    c, {}, None
                ),
            )
            bads = ["wat: 1\nname: x\nparams: p\nschema: s\n", "name: x\nparams: p\n", "- x\n"]
        ...
        self.assertEqual(
            expand_template(
                "{run.name}/{run.home}/{row.name}/{row.index}/{row.generation}/{row.x}",
                "r",
                "/h",
                {"x": 3},
                "rn",
        ...
        for t in ("{run.nope}", "{row.nope}", "{bad.x}"):
            with self.assertRaises(ConfigError):
                expand_template(t, "r", "h", {})

    def test_runconfig_validation(self):
        for kwargs in [
            {"name": "", "params": "p", "schema_source": {}},
            {"name": "bad/name", "params": "p", "schema_source": {}},
            {"name": "x", "params": "p", "schema_source": {}, "width": 0},
            {"name": "x", "params": "p", "schema_source": {}, "workers": -1},
            {"name": "x", "params": "p", "schema_source": {}, "scheduler": "x"},
            {"name": "x", "params": "p", "schema_source": {}, "terminal_level": "x"},
        ]:
            with self.assertRaises(ConfigError):
                RunConfig(**kwargs)
        with patch("jobchain.config.getpass.getuser", side_effect=RuntimeError), patch.dict(os.environ, {}, clear=True):
            self.assertEqual(expand_run_name("{user}"), "unknown")
...
class TestPipelineEdges(unittest.TestCase):
    def test_pipeline_build_and_chaining(self):
        ...
        self.assertEqual(p.chaining_stage, "job")
        self.assertEqual(p.spec("job").name, "job")
        with self.assertRaises(PipelineError):
            p.stage("x")
        with self.assertRaises(PipelineError):
            p.spec("x")
        with self.assertRaises(PipelineError):
            _build_pipeline({"stages": []}, "x")
        with self.assertRaises(PipelineError):
            _build_pipeline({"wat": 1, "stages": [{"name": "x"}]}, "x")
        with self.assertRaises(PipelineError):
            _build_pipeline({"defaults": 1, "stages": [{"name": "x"}]}, "x")
        with self.assertRaises(PipelineError):
            _build_pipeline({"stages": [{"name": "x"}, {"name": "x"}]}, "x")
        with self.assertRaises(PipelineError):
            _build_stage_spec("x", 1, {})
        with self.assertRaises(PipelineError):
            _build_stage_spec({}, 1, {})
        with self.assertRaises(PipelineError):
            _build_stage_spec({"name": "x", "depends": "bad"}, 1, {})
        specs = [_build_stage_spec({"name": "a"}, 1, {}), _build_stage_spec({"name": "b"}, 2, {})]
        _resolve_chaining(specs)
        self.assertEqual(specs[-1].depends, "afterany")
        specs = [
            _build_stage_spec({"name": "a"}, 1, {}),
            _build_stage_spec({"name": "b", "chains_next": True, "depends": "afterany"}, 2, {}),
        ]
        _resolve_chaining(specs)
        specs = [
            _build_stage_spec({"name": "a", "chains_next": True}, 1, {}),
            _build_stage_spec({"name": "b"}, 2, {}),
        ]
        _resolve_chaining(specs)
        with self.assertRaises(PipelineError):
            _resolve_chaining(
                [
                    _build_stage_spec({"name": "a", "chains_next": True}, 1, {}),
                    _build_stage_spec({"name": "b", "chains_next": True}, 2, {}),
                ]
            )
        with self.assertRaises(PipelineError):
            _resolve_chaining(
                [
                    _build_stage_spec({"name": "a"}, 1, {}),
                    _build_stage_spec(
                        {"name": "b", "chains_next": True, "depends": "afterok"}, 2, {}
                    ),
                ]
            )
    ...
    def test_pipeline_loading_and_stage_validation(self):
        with self.assertRaises(PipelineError):
            load_pipeline_source(1, "/tmp")
        with tempfile.TemporaryDirectory() as d:
            p = os.path.join(d, "p.yaml")
            with open(p, "w") as f:
                f.write("stages:\n - name: hello\n   command: echo hi\n")
            pipe = load_pipeline_source(p, d)
            self.assertEqual(pipe.name, "pipeline")
            with open(p, "w") as f:
                f.write("- bad\n")
            with self.assertRaises(PipelineError):
                load_pipeline_source(p, d)
        spec = _build_stage_spec({"name": "x", "command": "echo"}, 1, {})
        obj = _build_stage(spec, None, None, object())
        self.assertIsInstance(obj, JobStage)
        with self.assertRaises(PipelineError):
            _build_stage(_build_stage_spec({"name": "x"}, 1, {}), None, None, object())

        class Sg(JobStage):
            settings = {"n": Integer(required=True)}

        good = StageSpec("x", 1, "afterok", False, {"_position": 1, "n": 2}, "Sg")
        self.assertEqual(_validate_settings(good, Sg)["n"], 2)
        with self.assertRaises(PipelineError):
            _validate_settings(
                StageSpec("x", 1, "afterok", False, {"_position": 1, "bad": 2}, "Sg"), Sg
            )
        with self.assertRaises(PipelineError):
            _validate_settings(StageSpec("x", 1, "afterok", False, {"_position": 1}, "Sg"), Sg)
