"""Boundary and error-path unit coverage for config.py."""
...
from jobchain.config import ConfigError, RunConfig, expand_template, load_config, render_final_config


class TestRunConfigBoundaries(unittest.TestCase):
    def base(self, **overrides):
        values = dict(name="run", params="p", schema_source={"fields": []}, width=1,
                      workers=0, scheduler="pbs", terminal_level="info", file_level="debug")
        values.update(overrides)
        return RunConfig(**values)

    def test_invalid_run_name(self):
        with self.assertRaises(ConfigError): self.base(name="bad/name")

    def test_negative_workers(self):
        with self.assertRaises(ConfigError): self.base(workers=-1)

    def test_invalid_scheduler(self):
        with self.assertRaises(ConfigError): self.base(scheduler="local")

    def test_invalid_terminal_and_file_levels(self):
        with self.assertRaises(ConfigError): self.base(terminal_level="nope")
        with self.assertRaises(ConfigError): self.base(file_level="nope")
...
class TestTemplateBranches(unittest.TestCase):
    def test_date_time_and_username_placeholders(self):
        with patch("jobchain.config.time.strftime", side_effect=["2026-01-02", "030405"]), \
             patch("jobchain.config._username", return_value="alice"):
    ...
    def test_run_and_row_placeholders(self):
        value = expand_template("{run.name}/{run.home}/{row.name}/{row.index}/{row.x}/{row.generation}",
                                "r", "/tmp/r", {"x": "v"}, "000001", 3, 7)
...
class TestConfigFileErrors(unittest.TestCase):
    def test_missing_file(self):
        with self.assertRaises(ConfigError): load_config("/missing/jobchain.yaml")
    ...
    def base_config(self):
        return RunConfig(name="r", params="p", schema_source={"fields": []}, source_path="/tmp/c.yaml")
