"""Exhaustive unit tests for the remaining parser/normalizer branches."""
...
from jobchain.parse import (
    FieldFailure, LineChange, NormalizeResult, RowResult, ScanReport,
    _apply_file_validators, _check_header, _normalize_line, explain_row,
    format_change_report, format_report, join_fields, normalize_file, scan,
    split_line, write_normalized,
)


class TestParseRemaining(unittest.TestCase):
    def schema(self, **kw):
        base = dict(name="test", delimiter="|", quoting=False, fields=[], has_header=False,
                    comment_char="#", field_names=[], row_validators=[], file_validators=[])
        base.update(kw)
    ...
    def test_split_quoted_csv_error(self):
        ...
        class BrokenReader:
            def __next__(self): raise __import__("csv").Error("bad quote")
        with patch("jobchain.parse.csv.reader",return_value=BrokenReader()):
            with self.assertRaises(StructureError): split_line('"bad',schema)

    def test_normalize_missing_and_decode_and_os_errors(self):
        schema=self.schema()
        with patch("builtins.open",side_effect=FileNotFoundError):
            with self.assertRaises(StructureError): normalize_file("x",schema)
        with patch("builtins.open",side_effect=UnicodeDecodeError("utf-8",b"x",0,1,"bad")):
            with self.assertRaises(StructureError): normalize_file("x",schema)
    ...
    def test_normalize_header_and_mismatch(self):
        ...
        try:
            with patch("jobchain.parse._check_header") as check:
                result=normalize_file(path,schema)
            self.assertEqual(result.header,["a","x"]); check.assert_called_once()
        finally: os.unlink(path)

    def test_normalize_requires_header_when_file_empty(self):
        schema=self.schema(fields=[1],field_names=["a"],has_header=True)
        with tempfile.NamedTemporaryFile(mode="w",delete=False) as f: path=f.name
        try:
            with self.assertRaises(StructureError): normalize_file(path,schema)
        finally: os.unlink(path)
    ...
    def test_write_normalized_writes_header_and_rows(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(path) as h: self.assertEqual(h.read(),"a|b\n1|2\n3|4\n")

    def test_change_report_empty_and_truncated(self):
        self.assertEqual(format_change_report(NormalizeResult()),["No lines required normalization."])
    ...
    def test_row_failure_id_all_categories(self):
        ...
        r.structural_failure="bad"; self.assertEqual(r.failure_id(),"struct")
        r.structural_failure=None; r.field_failures=[FieldFailure("a",2,"x","d","bad"),FieldFailure("b",4,"x","d","bad")]
        self.assertEqual(r.failure_id(),"2+4")
        r.field_failures=[]; r.row_failures=["bad"]; self.assertEqual(r.failure_id(),"row")
        r.row_failures=[]; r.file_failures=["bad"]; self.assertEqual(r.failure_id(),"file")

    def test_row_reasons_combines_every_failure(self):
        r=RowResult(1,0,["x"],structural_failure="shape",field_failures=[FieldFailure("a",0,"x","int","bad")],row_failures=["rowbad"],file_failures=["filebad"])
        reasons=r.reasons()
        self.assertEqual(len(reasons),4); self.assertIn("a: bad",reasons[1])

    def test_scan_structural_field_and_row_paths(self):
        class V:
            description="check"
            def __init__(self,ok=True): self.ok=ok
            def validate(self,raw): return SimpleNamespace(ok=self.ok,value=int(raw) if self.ok else None,reason="bad")
    ...
    def test_scan_field_failure_stops_later_validators(self):
        class V:
            description="bad"
            def validate(self,raw): return SimpleNamespace(ok=False,value=None,reason="bad")
        class Never:
            description="never"
            def validate(self,raw): raise AssertionError("should not execute")
    ...
    def test_apply_file_validators_no_validators_is_noop(self):
        report=ScanReport("s","p"); schema=self.schema(file_validators=[])
    ...
    def test_apply_file_validators_file_and_known_and_unknown_lines(self):
        validator=SimpleNamespace(check=lambda candidates:[(0,"whole file"),(2,"row two"),(99,"ignored")])
        good=RowResult(2,0,["x"],record={"a":1})
        bad=RowResult(3,1,["x"],structural_failure="bad")
        report=ScanReport("s","p",rows=[good,bad]); schema=self.schema(file_validators=[validator])
        _apply_file_validators(report,schema)
    ...
    def test_explain_structural_short_circuit(self):
        ...
        self.assertIn("STRUCTURE",lines[1]); self.assertIn("raw fields",lines[2])

    def test_explain_no_checks_and_pass_and_fail(self):
        class V:
            description="check"
            def __init__(self,ok): self.ok=ok
            def validate(self,raw): return SimpleNamespace(ok=self.ok,value=7,reason="wrong")
        class RV:
            description="row check"
            def __init__(self,reason): self.reason_value=reason
            def check(self,record): return self.reason_value
        cols=[SimpleNamespace(name="plain",validators=[]),SimpleNamespace(name="ok",validators=[V(True)]),SimpleNamespace(name="bad",validators=[V(False)])]
        schema=self.schema(fields=cols,field_names=["plain","ok","bad"],row_validators=[RV(None),RV("rowbad")])
        r=RowResult(1,0,["x","1","2"],record={"plain":"x","ok":7,"bad":None},row_failures=[])
        lines=explain_row(r,schema)
        text="\n".join(lines)
        self.assertIn("no checks declared",text); self.assertIn("PASS",text); self.assertIn("FAIL",text); self.assertIn("value used",text); self.assertIn("-- rowbad",text)
    ...
    def test_format_report_optional_sections_and_limit(self):
        ...
        report=ScanReport("s","p",rows=bads,file_level_failures=["whole"],skipped_blank=1,skipped_comment=2)
        lines=format_report(report,limit=1)
        text="\n".join(lines)
        self.assertIn("Skipped 1 blank and 2 comment",text); self.assertIn("FILE: whole",text); self.assertIn("... and 2 more",text)
    ...
    def test_join_fields_whitespace_and_quoted(self):
        ...
        self.assertEqual(join_fields(["a,b","c"],self.schema(delimiter=",",quoting=True)), '"a,b",c')

    def test_normalize_line_reports_trim_reason(self):
        fields,reasons=_normalize_line(" a | b ",self.schema())
        self.assertEqual(fields,["a","b"]); self.assertIn("trimmed whitespace",reasons[0])
...
class TestParseFinalBranches(unittest.TestCase):
    ...
    def test_scan_row_validator_pass_records_typed_record(self):
        class V:
            description="int"
            def validate(self,raw): return SimpleNamespace(ok=True,value=1,reason=None)
