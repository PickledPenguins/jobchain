"""Exhaustive coverage of remaining schema validator/loader branches."""
...
from jobchain.schema import (
    AllOf, AnyOf, Field, Float, Int, OutputPath, PathExists, PredicateFile,
    PredicateRow, Schema, SchemaBase, Str, Validator, CheckResult,
    _build_field, _build_file_check, _build_row_check, _load_python_object,
    _load_python_row_validator, _load_python_validator, _load_schema_class,
    _resolve_delimiter, load_schema_source,
)
...
class TestSchemaRemaining(unittest.TestCase):
    def test_float_description_variants(self):
        ...
        with self.assertRaises(ValueError): Float(min=2,max=1)

    def test_path_exists_readability_and_normalize(self):
        with tempfile.TemporaryDirectory() as d:
            file=os.path.join(d,"f"); open(file,"w").close()
            v=PathExists(must_be_file=True,readable=True); v.base_dir=d
            self.assertEqual(v.normalize("f"),file); self.assertTrue(v.validate(file).ok)
    ...
    def test_schema_base_default_hooks_and_overrides(self):
        base=SchemaBase(); self.assertIsNone(base.check_row({})); self.assertEqual(base.check_file([]),[])
        class Custom(SchemaBase):
            fields=[Field("x",[Int()])]
            def check_row(self,row): return "bad" if row["x"] == 1 else None
            def check_file(self,rows): return [(2,"badfile")]
        schema=Custom().build(name="c")
        self.assertEqual(len(schema.row_validators),1); self.assertEqual(len(schema.file_validators),1)
    ...
    def test_schema_base_default_build_has_no_predicates(self):
        ...
        self.assertEqual(schema.row_validators,[]); self.assertEqual(schema.file_validators,[])
    ...
    def test_load_schema_source_mapping_absolute_and_relative_and_bad_type(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with self.assertRaises(SchemaError): load_schema_source(123,d,label="x")

    def test_yaml_loader_os_error(self):
        with tempfile.NamedTemporaryFile(delete=False,suffix=".yaml") as f: path=f.name
        try:
            with patch("builtins.open",side_effect=OSError("denied")):
                from jobchain.schema import load_schema
                with self.assertRaises(SchemaError): load_schema(path)
        finally: os.unlink(path)

    def test_field_non_mapping_and_optional_without_checks(self):
        with self.assertRaises(SchemaError): _build_field("x",0,"s")
    ...
    def test_row_and_file_check_shape_and_unknown_errors(self):
        with self.assertRaises(SchemaError): _build_row_check("x","s")
        with self.assertRaises(SchemaError): _build_row_check({"type":"missing"},"s")
        with self.assertRaises(SchemaError): _build_file_check("x","s")
        with self.assertRaises(SchemaError): _build_file_check({"type":"missing"},"s")

    def test_row_and_file_check_invalid_arguments(self):
        with self.assertRaises(SchemaError): _build_row_check({"type":"compare"},"s")
        with self.assertRaises(SchemaError): _build_file_check({"type":"unique","fields":1},"s")

    def test_python_row_check_path(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"v.py")
            with open(p,"w") as h:
                h.write("from jobchain.schema import RowValidator\nclass R(RowValidator):\n def __init__(self): super().__init__('r')\n def check(self,r): return None\nR=R()\n")
    ...
    def test_python_validator_wrong_type(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"v.py")
            with open(p,"w") as h: h.write("X=1\n")
            with self.assertRaises(SchemaError): _load_python_validator("v.py:X",p)
            with self.assertRaises(SchemaError): _load_python_row_validator({"ref":"v.py:X"},p)

    def test_python_object_invalid_reference_and_missing_file(self):
        with self.assertRaises(SchemaError): _load_python_object("bad", "/tmp/s.yaml")
        with self.assertRaises(SchemaError): _load_python_object("missing.py:X", "/tmp/s.yaml")

    def test_python_object_missing_attribute(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"v.py")
            with open(p,"w") as h: h.write("X=1\n")
            with self.assertRaises(SchemaError): _load_python_object("v.py:Y",p)

    def test_python_object_execution_error(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"v.py")
            with open(p,"w") as h: h.write("raise RuntimeError('boom')\n")
            with self.assertRaises(SchemaError): _load_python_object("v.py:X",p)
    ...
    def test_delimiter_none_and_literal(self):
        self.assertIsNone(_resolve_delimiter(None)); self.assertEqual(_resolve_delimiter("|"),"|")

    def test_load_schema_class_missing_and_ambiguous(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"classes.py")
            with open(p,"w") as h:
                h.write("from jobchain.schema import SchemaBase, Field, Str\nclass A(SchemaBase): fields=[Field('x',[Str()])]\nclass B(SchemaBase): fields=[Field('x',[Str()])]\n")
            with self.assertRaises(SchemaError): _load_schema_class("classes.py:C",p,{},"s",{})
            with self.assertRaises(SchemaError): _load_schema_class("classes.py",p,{},"s",{})

    def test_load_schema_class_explicit_and_single(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"classes.py")
            with open(p,"w") as h: h.write("from jobchain.schema import SchemaBase, Field, Str\nclass A(SchemaBase): fields=[Field('x',[Str()])]\n")
    ...
    def test_predicate_file_empty_return(self):
        p=PredicateFile(lambda rows:None,"x"); self.assertEqual(p.check([]),[])
...
class TestSchemaFinalBranches(unittest.TestCase):
    ...
    def test_python_schema_spec_loader_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(p,"w") as h: h.write("from jobchain.schema import Schema, Field, Str\nSCHEMA=Schema(fields=[Field('x',[Str()])])\n")
            with patch("jobchain.schema.importlib.util.spec_from_file_location",return_value=None):
                from jobchain.schema import _load_python_schema
                with self.assertRaises(SchemaError): _load_python_schema(p)

    def test_schema_class_module_without_schema_base(self):
        with tempfile.TemporaryDirectory() as d:
            p=os.path.join(d,"classes.py")
            with open(p,"w") as h: h.write("VALUE=1\n")
            with self.assertRaises(SchemaError): _load_schema_class("classes.py",p,{},"s",{})
    ...
    def test_import_module_spec_and_execution_failures(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(p,"w") as h: h.write("X=1\n")
            with patch("jobchain.schema.importlib.util.spec_from_file_location",return_value=None):
                with self.assertRaises(SchemaError): _import_module("m.py",p)
            with open(p,"w") as h: h.write("raise RuntimeError('boom')\n")
            with self.assertRaises(SchemaError): _import_module("m.py",p)
