"""Deep, mock-heavy unit coverage of jobchain/cli.py.

Renamed from test_cli_deep.py for this project's one-file-per-
...
class TestSharedHelpers(unittest.TestCase):
    ...
    def test_select_store_requires_root(self):
        with patch.object(cli.Store, "discover_root", return_value=None), self.assertRaises(StateError):
    ...
    def test_select_store_uses_environment_selector(self):
        with patch.dict(os.environ, {"JOBCHAIN_RUN": "a"}), patch.object(
            cli.Store, "discover_root", return_value="/r"
    ...
    def test_resolve_rows_by_status(self):
        ...
        prepared = SimpleNamespace(
            store=MagicMock(load_rows=MagicMock(return_value=rows)),
            schema=SimpleNamespace(unique_fields=[]),
        )
    ...
    def test_resolve_rows_all_active(self):
        rows = [
            SimpleNamespace(current=1, is_terminal=False),
            SimpleNamespace(current=1, is_terminal=True),
            SimpleNamespace(current=None, is_terminal=False),
        ]
        prepared = SimpleNamespace(
            store=MagicMock(load_rows=MagicMock(return_value=rows)),
            schema=SimpleNamespace(unique_fields=[]),
        ...
        self.assertEqual(
            cli._resolve_rows(prepared, ns(rows=[], statuses=[], all_rows=True)), [rows[0]]
        )

    def test_resolve_rows_default_active(self):
        row = SimpleNamespace(current=1, is_terminal=False)
        prepared = SimpleNamespace(
            store=MagicMock(load_rows=MagicMock(return_value=[row])),
            schema=SimpleNamespace(unique_fields=[]),
        )
    ...
    def test_resolve_rows_requires_selection(self):
        prepared = SimpleNamespace(
            store=MagicMock(load_rows=MagicMock(return_value=[])),
            schema=SimpleNamespace(unique_fields=[]),
        )
...
class TestProgress(unittest.TestCase):
    ...
    def test_progress_paints_when_terminal(self):
        fake = io.StringIO()
        with patch.object(cli.sys, "stderr", fake), patch.object(
            cli.sys.stderr, "isatty", return_value=True
        ):
...
class TestShowAndOutput(unittest.TestCase):
    ...
    def test_show_paths_and_stages(self):
        row = SimpleNamespace(
            current=SimpleNamespace(stages=[]),
            name="a",
            row_id="1",
            line_num=1,
            status="DONE",
    ...
    def test_show_json(self):
        stage = SimpleNamespace(
            name="s", status="DONE", jobid="1", depends="", script="x", resources={}, error=None
        )
    ...
    def test_show_output_requires_stage(self):
        with self.assertRaises(StateError):
            cli._show_output(
                SimpleNamespace(store=SimpleNamespace(home="/h")),
                SimpleNamespace(name="a", stage_reached=None),
                None,
            )
    ...
    def test_show_output_missing_log(self):
        with self.assertRaises(StateError):
            cli._show_output(
                SimpleNamespace(store=SimpleNamespace(home="/h")),
                SimpleNamespace(name="a", stage_reached="s"),
                None,
            )
...
class TestRerunCancelDoctorLogsExport(unittest.TestCase):
    ...
    def test_doctor_all_json(self):
        ...
        with patch.object(cli.Store, "discover_root", return_value="/r"), patch.object(
            ...
            self.assertEqual(
                cli.cmd_doctor(
                    ns(check_fs=False, all_runs=True, repair=False, dry_run=True, as_json=True)
                ),
                EXIT_OK,
            )
    ...
    def test_doctor_single_bad_text_and_repair(self):
        ...
        with patch.object(cli.commands, "_open", return_value=prepared), patch.object(
            ...
            self.assertEqual(
                cli.cmd_doctor(
                    ns(check_fs=False, all_runs=False, repair=True, dry_run=False, as_json=False)
                ),
                EXIT_OK,
            )
...
class TestCliRemainingBranches(unittest.TestCase):
    ...
    def test_cmd_run_text_executes_header_and_completion(self):
        ...
        result = SimpleNamespace(
            store=SimpleNamespace(name="r", home="/home"),
            phase="run",
            rows_created=2,
            rows_invalid=0,
            scripts_written=1,
    ...
    def test_cmd_status_row_text_and_metrics(self):
        ...
        prepared = SimpleNamespace(
            store=store,
            schema=SimpleNamespace(unique_fields=["rid"]),
    ...
    def test_status_body_all_optional_sections(self):
        ...
        with patch.object(cli.report, "render_summary", return_value=["summary"]), patch.object(
            cli.report, "render_warnings", return_value=["warning"]
        ), patch.object(cli.report, "render_metrics", return_value=["metrics"]), patch.object(
            cli.report, "render_table", return_value=["table"]
        ):
            lines = cli._status_body(
                prepared, [], [], {}, metrics, ns(metrics=True, summary_only=False)
            )
        self.assertIn("warning", lines)
        self.assertIn("metrics", lines)
    ...
    def test_status_body_omits_warnings_metrics_table(self):
        ...
        with patch.object(cli.report, "render_summary", return_value=["summary"]), patch.object(
            cli.report, "render_warnings", return_value=[]
        ), patch.object(cli.report, "render_metrics") as rm, patch.object(
            cli.report, "render_table"
        ) as rt:
            lines = cli._status_body(
                prepared, [], [], {}, metrics, ns(metrics=False, summary_only=True)
            )
    ...
    def test_watch_completes_after_terminal_rows(self):
        ...
        store.load_config.return_value = {"width": 1}
        prepared = SimpleNamespace(store=store, config=SimpleNamespace(description=""))
        with patch.object(cli.report, "build_views", return_value=[]), patch.object(
            cli.report, "filter_views", return_value=[]
    ...
    def test_logs_empty_matching_text(self):
        ...
        try:
            with patch.object(cli.commands, "_open", return_value=prepared), patch.object(
                ...
                self.assertEqual(
                    cli.cmd_logs(
                        ns(follow=False, level="ERROR", stage=None, lines=40, as_json=False)
                    ),
                    EXIT_OK,
                )
...
class TestCliLastGaps(unittest.TestCase):
    def test_attach_file_log_uses_argument_and_config_levels(self):
        prepared = SimpleNamespace(
            store=SimpleNamespace(log_path="/run.log"),
    ...
    def test_show_output_branch(self):
        ...
        output.assert_called_once_with(prepared, row, "solve")
    ...
    def test_doctor_single_json_success(self):
        ...
        with patch.object(cli.commands, "_open", return_value=prepared), patch.object(
            ...
            self.assertEqual(
                cli.cmd_doctor(
                    ns(check_fs=False, all_runs=False, repair=False, dry_run=False, as_json=True)
                ),
                EXIT_OK,
            )
        emit.assert_called_once_with({"ok": True})
        payload.assert_called_once_with(result)
    ...
    def test_watch_sleeps_for_active_run_then_completes(self):
        ...
        store = MagicMock()
        store.load_rows.side_effect = [[active], [done]]
        store.load_config.return_value = {"width": 1}
        prepared = SimpleNamespace(store=store, config=SimpleNamespace(description=""))
...
class TestCliFinalBranches(unittest.TestCase):
    ...
    def test_rerun_confirmation_accepts(self):
        prepared = SimpleNamespace(
            store=MagicMock(name="store"), config=SimpleNamespace(on_complete=None)
        )
